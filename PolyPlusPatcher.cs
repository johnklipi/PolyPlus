...
namespace PolyPlus
{
    public static class PolyPlusPatcher
    {
        ...
        private static void UnitData_getPromotionLimit(
            ref int __result,
            UnitData __instance,
            PlayerState player,
            GameState gameState
        ...
        {
            if (
                __instance.unitAbilities.Contains(EnumCache<UnitAbility.Type>.GetType("staticplus"))
            )
            {
                __result = 0;
            }
        ...
        private static void MapGenerator_SetTileAsCapital(
            GameState gameState,
        ...
        )
        {
            ...
            {
                if (
                    tribeData.tribeAbilities.Contains(
                        EnumCache<TribeAbility.Type>.GetType("citypark")
                    )
                )
                {
        ...
        private static void PathFinder_IsTileAccessible(
            ref bool __result,
            TileData tile,
            TileData origin,
            ...
            )
            {
                if (
                    (
                        ...
                        && settings.allowedTerrain.Contains(Polytopia.Data.TerrainData.Type.Water)
                    )
                    || (
                        tile.terrain == Polytopia.Data.TerrainData.Type.Ocean
                        && settings.allowedTerrain.Contains(Polytopia.Data.TerrainData.Type.Ocean)
                    )
                )
                {
                    __result = true;
                        ...
                        if (!startTile.IsWater)
                        {
                            ...
                            foreach (WorldCoordinates pathTile in path)
                            {
                                ...
                                )
                                {
                                    ...
                                    {
                                        if (
                                            gameState.GameLogicData.TryGetData(
                                                tile.improvement.type,
                                                out ImprovementData imrovementData
                                            )
                                        )
                                        {
                                            if (
                                                !imrovementData.HasAbility(
                                                    ImprovementAbility.Type.Bridge
                                                )
                                            )
                                            {
                                                hadWater = true;
            ...
            )
            {
                ...
                )
                {
                    gameState.ActionStack.Add(
                        new EmbarkAction(__instance.PlayerId, worldCoordinates)
                    );
                }
            }
        }
        ...
        private static void EmbarkAction_ExecuteDefault(
            EmbarkAction __instance,
            GameState gameState
        )
        {
            PlayerState playerState;
            if (gameState.TryGetPlayer(__instance.PlayerId, out playerState))
            {
                ...
                if (
                    PlayerExtensions.HasAbility(
                        playerState,
                        EnumCache<PlayerAbility.Type>.GetType("dashembark"),
                        gameState
                    )
                )
                {
        ...
        private static void ActionUtils_CalculateImprovementLevel(
            ref int __result,
            GameState gameState,
            TileData tile
        )
        {
            if (tile.improvement != null)
            {
                ...
                {
                    if (
                        improvementData.HasAbility(
                            EnumCache<ImprovementAbility.Type>.GetType("halved")
                        )
                    )
                    {
                        __result /= 2;
        ...
        [HarmonyPostfix]
        [HarmonyPatch(
            typeof(PlayerDiplomacyExtensions),
            nameof(PlayerDiplomacyExtensions.GetIncomeFromEmbassy)
        )]
        private static void PlayerDiplomacyExtensions_GetIncomeFromEmbassy(
            ref int __result,
            PlayerState playerState,
        ...
        private static void TileData_GetMovementCost(
            ref int __result,
            TileData __instance,
            MapData map,
            TileData fromTile,
        ...
        private static bool MapDataExtensions_UpdateRoutes_Prefix(
            GameState gameState,
        ...
        private static void GameLogicData_GetMovementsWithUnlockedTeck(
            ref Il2CppSystem.Collections.Generic.List<Polytopia.Data.TerrainData> __result,
            GameLogicData __instance,
            Il2CppSystem.Collections.Generic.List<TechData> tech
        )
        {
            if (unlockRoutes)
            {
        ...
        [HarmonyPostfix]
        [HarmonyPatch(typeof(MapDataExtensions), nameof(MapDataExtensions.UpdateRoutes))]
        private static void MapDataExtensions_UpdateRoutes_Postfix(
            GameState gameState,
            Il2CppSystem.Collections.Generic.List<TileData> changedTiles
        )
        {
        ...
        [HarmonyPostfix]
        [HarmonyPatch(
            typeof(UnitDataExtensions),
            nameof(UnitDataExtensions.GetAttackOptionsAtPosition)
        )]
        private static void GetAttackOptionsAtPosition(
            ...
            int range,
            bool includeHiddenTiles,
            UnitState customUnitState,
            bool ignoreDiplomacyRelation
        )
        {
        ...
        [HarmonyPrefix]
        [HarmonyPatch(
            typeof(TileDataExtensions),
            nameof(TileDataExtensions.CalculateWork),
            typeof(TileData),
            typeof(GameState),
            typeof(int)
        )]
        public static bool TileDataExtensions_CalculateWork_Prefix(
            ref int __result,
            TileData tile,
            GameState gameState,
        ...
        public static void ImprovementState_HasEffect(
            ref bool __result,
            ImprovementState __instance,
        ...
        public static void TileDataExtensions_CalculateWork_Postfix(
            ref int __result,
            TileData tile,
            GameState gameState,
            int improvementLevel
        ...
        [HarmonyPrefix]
        [HarmonyPatch(
            typeof(InfiltrationRewardAction),
            nameof(InfiltrationRewardAction.ExecuteDefault)
        )]
        private static bool InfiltrationRewardAction_ExecuteDefault_Prefix(
            InfiltrationRewardAction __instance,
            GameState gameState
        )
        {
        ...
        [HarmonyPatch(
            typeof(TileDataExtensions),
            nameof(TileDataExtensions.CalculateRawProduction)
        )]
        private static void TileDataExtensions_CalculateRawProduction(
            ref int __result,
            TileData tile,
            GameState gameState
        )
        ...
        private static void InfiltrationRewardAction_ExecuteDefault_Postfix(
            InfiltrationRewardAction __instance,
            GameState gameState
        )
        {
        ...
        [HarmonyPrefix]
        [HarmonyPatch(
            typeof(InfiltrationRewardReaction),
            nameof(InfiltrationRewardReaction.Execute)
        )]
        private static bool InfiltrationRewardReaction_Execute(
            InfiltrationRewardReaction __instance,
        ...
        )
        {
            ...
            {
                new(
                    Localization.Get("buttons.ok"),
                    PopupButtonData.States.None,
                    (UIButtonBase.ButtonAction)OnOkInfiltrationClicked,
                    -1,
                    true,
                    null
                ),
